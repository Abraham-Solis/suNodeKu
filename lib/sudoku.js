...
class Puzzle {
    ...
    compress() {
        ...
        for(let index = 0; index < 11; index++) {
            ...
            for(let bitIndex = 0; bitIndex < 8; bitIndex++) {
                value |= (this.given[(index * 8) + bitIndex] & 0x1) << bitIndex;
            }
            data[GIVEN_OFFSET + index] = value;
            //console.log(`${index}:${index*8} - Comp given ${data[GIVEN_OFFSET + index].toString(2)}`);
        }

        // compress the blank mask
        for(let index = 0; index < 11; index++) {
            let value = 0;
            for(let bitIndex = 0; bitIndex < 8; bitIndex++) {
                value |= (this.blank[(index * 8) + bitIndex] & 0x1) << bitIndex;
            }
            data[BLANK_OFFSET + index] = value;
            //console.log(`${index}:${index*8} - Comp given ${data[GIVEN_OFFSET + index].toString(2)}`);
        }
    ...
    decompress(data) {
        ...
        // Parse the mask that says which cells are blank
        for(let index = 0; index < 11; index++) {
            for(let bitIndex = 0; bitIndex < 8; bitIndex++) {
                if(index===10 && bitIndex >= 4) break;
                let value = data[BLANK_OFFSET + (index)];
                //console.log(`${(index*8)} - ${value.toString(2)}`);
