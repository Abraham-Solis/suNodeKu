...
class Puzzle {
    ...
    compress() {
        ...
        let destIndex = 0;
        for(let index = 0; index < 82; index += 2) {
    ...
    decompress(data) {
        ...
        for(let index = 0; index < 81; index += 2) {
            ...
            if(this.blank[index + 0]) {
                this.board[index + 0] = 255;
            } else {
                this.board[index + 0] = cellA;
            }

            if(this.blank[index + 1]) {
                this.board[index + 1] = 255;
            } else {
                this.board[index + 1] = cellB;
            }

            // decompress solution
            value = data[SOLUTION_OFFSET + srcIndex];
            cellA = (value & 0xF);
            cellB = ((value >> 4) & 0xF);
